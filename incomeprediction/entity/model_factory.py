...
def evaluate_classification_model(model_list: list, X_train:np.ndarray, y_train:np.ndarray, X_test:np.ndarray, y_test:np.ndarray, base_accuracy:float=0.6)->MetricInfoArtifact:
    ...
    try:
        ...
        for model in model_list:
            ...
            train_accuracy_score =  round(accuracy_score(y_train, y_train_pred) * 100, 3)
            test_accuracy_score =  round(accuracy_score(y_test, y_test_pred) * 100, 3)

            #Calculating F1 score on training and testing dataset
            train_F1_Score = round(f1_score(y_train, y_train_pred) * 100,3)
            test_F1_Score = round(f1_score(y_test,y_test_pred) * 100,3)
